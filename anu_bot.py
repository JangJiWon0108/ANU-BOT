...
def run_jumpit_wanted_review_page():
    time.sleep(1)
    subprocess.Popen(["streamlit", "run", "jumpit_wanted_review.py"])

# 다른 .py 파일 실행 (에타 강의평 1)
def run_everytime_review_page_1():
    time.sleep(1)
    subprocess.Popen(["streamlit", "run", "everytime_review_1.py"])

# 다른 .py 파일 실행 (에타 강의평 2)
def run_everytime_review_page_2():
    time.sleep(1)
    subprocess.Popen(["streamlit", "run", "everytime_review_2.py"])

# 다른 .py 파일 실행 (에타 강의평 3)
def run_everytime_review_page_3():
    time.sleep(1)
    subprocess.Popen(["streamlit", "run", "everytime_review_3.py"])

# 다른 .py 파일 실행 (안동대 정보 챗봇)
def run_anu_info_page():
    time.sleep(1)
    subprocess.Popen(["streamlit", "run", "anu_bot_info.py"])
...
# =================================== 사이드 바 =============================   
with st.sidebar:
    selected = option_menu("아누봇 메뉴", ["안동대학교 정보", "교과목 정보", "채용공고 정보"], 
